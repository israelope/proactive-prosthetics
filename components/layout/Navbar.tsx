...
export default function Navbar() {
  ...
  // State for styling (background color)
  const [isScrolled, setIsScrolled] = useState(false);
  
  // NEW: State to track if the navbar should be visible or hidden
  const [isVisible, setIsVisible] = useState(true);
  
  // NEW: We use a ref to store the last scroll position without causing re-renders
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // 1. Handle Background Style (Past 50px)
      ...
      // 2. Handle Hide/Show Visibility
      // If scrolling DOWN and we are past the very top of the page -> Hide
